...
@WebServlet(name = "SetMasterServlet", value = "/manager/set/master")
public class SetMasterServlet extends HttpServlet {
    private final OrderService orderService;

    public SetMasterServlet() throws SQLException {
        orderService = new OrderService(new OrderDaoImpl(DatabaseConnection.getDBConnection().getConnection()));
    }

    @Override
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        request.getRequestDispatcher("/managerSetMaster.jsp").forward(request, response);
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            orderService.setMaster(orderId, master);
            response.sendRedirect("/workshop_war_exploded/orders/profile");
        } catch (SQLException throwables) {
            response.sendRedirect("/workshop_war_exploded/manager/set/master");
        }
    }
}
